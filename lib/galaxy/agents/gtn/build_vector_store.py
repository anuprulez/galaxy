#!/usr/bin/env python3
"""
Build GTN Vector Store from GTN Repository.

This script processes the Galaxy Training Network repository and creates
a vector store with embeddings for fast semantic search using LlamaIndex.

Usage:
    python build_vector_store.py <gtn_repo_path> [--output <store_path>]
"""
...
class GTNVectorStoreBuilder:
    """Builds vector store from GTN repository."""

    def __init__(self, gtn_path: Path, output_path: Optional[Path] = None):
        self.gtn_path = gtn_path
        self.output_path = output_path or (Path(__file__).parent / "data" / "gtn_vector_store")
    ...
    def build(self):
        ...
        log.info(f"Building GTN vector store from {self.gtn_path}")
        ...
        log.info(f"Vector store built successfully: {self.output_path}")
    ...
    def create_documents(self):
        """Create LlamaIndex documents from tutorials and FAQs."""
        for tutorial in self.tutorials:
            ...
            doc = Document(
                text=text,
                metadata=metadata,
                id_=f"tutorial_{tutorial.topic}_{tutorial.tutorial}"
            )
        ...
        for faq in self.faqs:
            ...
            doc = Document(
                text=text,
                metadata=metadata,
                id_=f"faq_{faq.category}_{faq.filename}"
            )
            self.documents.append(doc)
    ...
    def create_vector_store(self):
        ...
        # Create storage context (new, not loading from disk)
        ...
        # Create index from documents (embed_model=None uses LlamaIndex default)
        ...
        log.info(f"Vector store created with {len(self.documents)} documents")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Build GTN Vector Store from GTN Repository")
    parser.add_argument("gtn_repo_path", type=str, help="Path to cloned GTN repository")
    parser.add_argument("--output", type=str, help="Output vector store path (default: data/gtn_vector_store relative to this script)")
    ...
    try:
        builder = GTNVectorStoreBuilder(gtn_path, output_path)
        builder.build()

    except Exception as e:
        log.error(f"Failed to build vector store: {e}", exc_info=True)
        sys.exit(1)
